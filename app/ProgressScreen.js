...
export default function ProgressScreen() {
  ...
  const updateProgress = async (itemId, increment) => {
    ...
    setTrackedItems(prev => prev.map(i => i.itemId === itemId ? { ...i, current: newCurrent } : i));
    try {
      await fetch(`${IP}/progress`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId: Number(userId), itemId, current: newCurrent })
      });
    } catch (e) {
  ...
  const removeItem = async (itemId) => {
    ...
    try {
      ...
      showToast("Error removing progress");
    }
  };
  ...
  return (
    <View style={[styles.background, { backgroundColor: theme.fundal }]}>
      <View style={styles.headerContainer}>
        <Text style={[styles.pageTitle, { color: theme.accent }]}>My Progress</Text>
      ...
      <FlatList
        ...
        ListEmptyComponent={<Text style={[styles.emptyText, { color: theme.textSecundar }]}>No progress tracked yet.</Text>}
        renderItem={({ item }) => {
          const progressPercent = item.total ? Math.min((item.current / item.total) * 100, 100) : 0;
          return (
            <View style={[styles.card, { backgroundColor: theme.card, borderColor: theme.bordura }]}>
              <Image source={{ uri: item.image }} style={[styles.image, { borderColor: theme.bordura }]} />
              <View style={styles.infoContainer}>
                ...
                <Text style={[styles.subtitle, { color: theme.textSecundar }]}>
                  {item.type === 'anime' ? 'Episodes' : 'Chapters'}: <Text style={{color: theme.text}}>{item.current}</Text> / {item.total || '?'}
                </Text>
                <View style={[styles.progressBarBackground, { backgroundColor: theme.bordura }]}>
                  <View style={[styles.progressBarFill, { width: `${progressPercent}%`, backgroundColor: theme.accent }]} />
                </View>
                <View style={styles.controlsContainer}>
                  <TouchableOpacity style={[styles.controlButton, { backgroundColor: theme.card, borderColor: theme.bordura }]} onPress={() => updateProgress(item.itemId, -1)} disabled={item.current === 0}>
                    <Ionicons name="remove" size={24} color={item.current === 0 ? theme.tabInactiv : theme.accent} />
                  </TouchableOpacity>
                  <TouchableOpacity style={[styles.controlButton, { backgroundColor: theme.card, borderColor: theme.bordura }]} onPress={() => updateProgress(item.itemId, 1)} disabled={item.total && item.current >= item.total}>
                    <Ionicons name="add" size={24} color={(item.total && item.current >= item.total) ? theme.tabInactiv : theme.accent} />
                  </TouchableOpacity>
                </View>
              </View>
            </View>
